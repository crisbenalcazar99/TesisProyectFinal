...
demo = {
  ...
  var myChart = new Chart(ctx, {
    ...
    responsive: true,
    data: {
      labels: [], // Inicialmente vacío, se llenará con datos de la API
      datasets: [{
        label: "Temperatura",
        borderColor: "#f96332",
  ...
  function updateChart() {
    fetch('/get-temperature-data/') // Reemplaza con la URL correcta de tu API
      .then(response => response.json())
      .then(data => {
        myChart.data.labels = data.labels; // Actualizar etiquetas
        myChart.data.datasets[0].data = data.temperatures; // Actualizar datos de temperatura
        myChart.data.datasets[1].data = data.humidities; // Actualizar datos de humedad

        myChart.update(); // Actualizar gráfico
      })
      .catch(error => {
  ...
  // Actualizar gráfico al iniciar
  updateChart();

  // Configurar actualización cada 2 minutos (120,000 ms)
  ...
  initDashboardPageCharts: function() {
    ...
    // Configuración de los gráficos con tooltip
    ...
    gradientStroke.addColorStop(0, 'rgba(119,52,169,0)'); // Colores púrpura
...
function updateTemperatureChart() {
  fetch('/get-temperature-data/')
    ...
    .then(data => {
      ...
      // Actualizar etiquetas con fechas
...
setInterval(updateTemperatureChart, 300000);
